...
public class Main {
    ...
    public static void bienvenida(Scanner scan) {
        ...
        System.out.println(
                "Presione 1 para crear una nueva Base de Datos (solo ingresan datos a la Base de Datos 'Prueba')");
        ...
        if (decision.equals("1")) { // Crear nueva BD con sus Tablas
            ...
            if (consulta.matches("SELECT \\* FROM <\\w+> WHERE <\\w+> == <\\w+>")) {
                ...
                Matcher matcher = Pattern.compile("<(\\w+)>").matcher(consulta); // Expresion regular para extraer los
                                                                                 // valores entre <>
