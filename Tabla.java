...
public class Tabla {
    ...
    private Map<String, Map<String, List<String[]>>> indices; // Index para columnas especificas (clave=nombreColumna,
                                                              // valor=indiceColumna)
    ...
    // Método para indexar una columna específica (se hace asi para poder aplicarlo
    // a las consultas)
    public void indexarColumna(String nombreColumna, String ruta) {
        Map<String, List<String[]>> indiceColumna = new HashMap<>(); // Index de valores en la columna
                                                                     // (clave=valoresenColumna, valor=fila completa en
                                                                     // la que coincide ese valor)
    ...
    public void generarTablaCSV(String nombre, String ruta) {
        try {
            ...
            lock.lock();
            try {
                // Las columnas de cada una de las tablas (se pueden agregar mas columnas
    ...
    // Insertar una fila en la tabla (se debe modificar) (se podria eliminar la ruta
    // y poner una ruta por default)
    public void agregarFilaCSV(String data, String nombreTabla) {
        try {
            ...
            indexarColumna("id" + nombreTabla, ruta); // Para poder indexarlas cuando se inserta un dato, guiandose del
                                                      // id de cada una
    ...
    public void consultar(String ruta, String nombreColumna, String valorBuscado) {
        ...
        Map<String, List<String[]>> indiceColumna = indices.get(nombreColumna); // Index de valores en la columna
                                                                                // (clave=valoresenColumna, valor=fila
                                                                                // completa en la que coincide ese
                                                                                // valor)
        ...
        List<String[]> resultados = indiceColumna.get(valorBuscado); // Lista que almacena las filas completas para
                                                                     // imprimirlas (despues de la busqueda)
        System.out.println(
                "Resultados encontrados para la columna '" + nombreColumna + "' con valor '" + valorBuscado + "':");
